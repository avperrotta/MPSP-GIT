...
MainContentComponent::MainContentComponent(){
    ...
    oscReceiver.setup(DEFAULT_OSC_RECEIVER_PORT);
...
//    currentFrameTime = juce::Time::getMillisecondCounter();
...
PopupMenu MainContentComponent::getMenuForIndex (int topLevelMenuIndex, const String &menuName){
    ...
    if (topLevelMenuIndex == 0)
    {
        ...
        menu.addItem(4, "Trajectory Editor");
        menu.addItem(5, "Spat config");
...
void MainContentComponent::showGlViewControls(bool show){
    frameRateLabel->setVisible(false);
    zoomSlider->setVisible(show);
    rollSlider->setVisible(show);
    pitchSlider->setVisible(show);
    yawSlider->setVisible(show);
    view1->setVisible(show);
    view2->setVisible(show);
    view3->setVisible(show);
    view4->setVisible(show);
}
