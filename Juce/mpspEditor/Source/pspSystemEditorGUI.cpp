...
pspSystemEditorGUI::pspSystemEditorGUI(pspParticleSystemsManager* psm){
    ...
    addSystemsMenuButton->setButtonText("Add Trajectory");
...
void pspSystemEditorGUI::populateAddSystemsMenu(){
    ...
    subMenu.addItem(103, "angular");
    subMenu.addItem(104, "sphericalMotion");
...
void pspSystemEditorGUI::addSystemsMenuItemSelected(int result, pspSystemEditorGUI* p){
    switch(result){
        case 101:
            ...
            break;
        case 102:
            p->getPsManager()->addSystem("linear", 1);
            p->systemsList.updateContent();
            p->systemsList.deselectAllRows();
            break;
        case 103:
            p->getPsManager()->addSystem("angular", 1);
            p->systemsList.updateContent();
            p->systemsList.deselectAllRows();
            break;
        case 104:
            p->getPsManager()->addSystem("sphericalMotion", 1);
            p->systemsList.updateContent();
            p->systemsList.deselectAllRows();
            break;
    }
